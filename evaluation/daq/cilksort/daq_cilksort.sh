...
compile_with_cutoff() {
    ...
    local GTAP_MAX_TASKS_PER_WARP="${GTAP_MAX_TASKS_PER_WARP:-90000}"
    local GTAP_CFLAGS="${GTAP_CFLAGS:-}"
    if [ -z "$CUDA_PATH" ]; then
    ...
    mkdir -p "$BIN_DIR"
    # Do not run a binary left over from a previous cutoff if this build fails.
    rm -f "$out_file"
    ...
    "$CXX_BIN" -O3 -x cuda \
      ...
      $extra_flags \
      -I"$GT_INC" \
      -DGTAP_DAQ_GRID_SIZE="$GTAP_GRID_SIZE" \
      -DGTAP_DAQ_BLOCK_SIZE="$GTAP_BLOCK_SIZE" \
      -DGTAP_DAQ_MAX_TASKS_PER_WARP="$GTAP_MAX_TASKS_PER_WARP" \
