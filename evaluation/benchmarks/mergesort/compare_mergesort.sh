...
DYNASOAR_MERGESORT_BIN="${DYNASOAR_MERGESORT_BIN:-${DYNASOAR_ROOT:+$DYNASOAR_ROOT/bin/MergeSort}}"
...
ensure_dynasoar_bin() {
    ...
    if [ -x "$DYNASOAR_MERGESORT_BIN" ]; then
        return 0
    fi
    echo "Building Dynasoar MergeSort in $DYNASOAR_ROOT ..."
    make -C "$DYNASOAR_ROOT" bin/MergeSort || {
        echo "Warning: Dynasoar MergeSort build failed; skipping Dynasoar runs" >&2
        return 1
    }
    [ -x "$DYNASOAR_MERGESORT_BIN" ]
}
...
if ensure_dynasoar_bin; then
    DYNASOAR_ENABLED=1
    echo "Dynasoar MergeSort: $DYNASOAR_MERGESORT_BIN"
else
    echo "Dynasoar MergeSort: disabled"
fi
...
for size in "${SIZES[@]}"; do
    ...
    if [ "$DYNASOAR_ENABLED" = "1" ]; then
        read DYNASOAR_MED _ _ DYNASOAR_ELO DYNASOAR_EHI < <(run_stats_dynasoar "$DYNASOAR_MERGESORT_BIN" "$TMP_DIR/$DATA_FILE")
    fi
