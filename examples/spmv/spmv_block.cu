...
// Divide-and-conquer SpMV: 1 block = 1 task
// range_spmv recursively splits the row range; leaf tasks compute one row
// using all threads in the block (parallel reduction over row elements).
...
__device__ void spmv_row(int row_idx) {
    ...
    for (int k = row_begin + threadIdx.x; k < row_end; k += blockDim.x) {
        s_sum[threadIdx.x] += g_val[k] * g_x[g_col_idx[k]];
    }
    ...
    if (threadIdx.x == 0) g_y[row_idx] = s_sum[0];
}

// Divide [begin, end) into halves; spawn spmv_row at leaves (thread 0 only)
#pragma gtap function
...
int main(int argc, char** argv) {
    ...
    st = gtap_launch(exec_kernel, rows);
    ...
    printf("Sum of y: %.6f\n", sum_y);
    // Block reduction accumulates floating-point errors across threads;
    // use a relative tolerance scaled by the row's magnitude.
