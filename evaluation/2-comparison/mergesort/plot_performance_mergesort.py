#!/usr/bin/env python3
"""Plot Mergesort: absolute time and GTaP-normalized time (combined figure)."""
...
if not csv_path.exists():
    raise SystemExit(f"Missing CSV: {csv_path} (run ./compare_mergesort.sh first)")
...
def configure_x_axis(ax, sub: pd.DataFrame, *, log_pad_frac: float = 0.06):
    """Log-x limits padded to data range; ticks at measured array sizes."""
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Plot Mergesort performance (GTaP vs CPU baselines).")
    ap.add_argument(
        "--include-seq",
        action="store_true",
