...
def compute_utilization_from_timeline(timeline_df, strong_state):
    ...
    for block_id, grp in timeline_df.groupby('block_id'):
        ...
        # Handle case where last state is still working
        if working_start_time is not None:
...
def create_timeline_plot(timeline_df, stats_df, strong_state, app_name=None, max_blocks=None):
    ...
    active_blocks = stats_df[stats_df['total_samples'] > 0]['block_id'].tolist()
    if max_blocks is not None:
        active_blocks = active_blocks[:max_blocks]
    ...
    ax.set_yticklabels([f'Block {bid}' for bid in active_blocks])
...
def main():
    parser = argparse.ArgumentParser(description='Block Timeline Visualization (Block Runtime)')
    parser.add_argument('--app_name', type=str, default='fib_plus', help='Prefix (app name) for CSV and image outputs')
    args = parser.parse_args()

    print("Block Timeline Visualization Tool (Block Runtime)")
    print("=" * 40)
    ...
    try:
        ...
        print(f"Make sure block_timeline_working.csv and block_statistics_working.csv exist in {args.app_name}/profile/ directory")
