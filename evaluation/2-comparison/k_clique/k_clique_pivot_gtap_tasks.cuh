#if GTAP_K_PIVOT_PREFIX_PASS_STATE
...
__device__ void edgek_pivot_shared_prefix_task(
    ...
    int m,
    int depth,
    KPivotPrefixState parent_state,
    int range_begin,
    int range_end) {
#ifdef GTAP_K_STATS
    ...
    if (parent_state.level != depth) return;
    if (parent_state.pl_count <= 0) return;
    load_pivot_prefix_state(parent_state, words);
    int parent_pl_count = parent_state.pl_count;
    int parent_rsize = parent_state.rsize;
    int parent_drop = parent_state.drop;
    int parent_pivot = parent_state.pivot;
    ...
    for (int ordinal = range_begin; ordinal < range_end; ++ordinal) {
        ...
        if (should_spawn) {
            KPivotPrefixState child_state;
            save_pivot_prefix_state(
                &child_state,
                child_level,
                child_pl_count,
                child_rsize,
            ...
            for (int s = 0; s < child_pl_count; s += chunk) {
                ...
                edgek_pivot_shared_prefix_task(
                    slot, u, v, m, child_level, child_state, s, e);
            }
        } else {
...
__device__ void edgek_pivot_shared_prefix_task(
    ...
    if (rebuild_status != K_PREFIX_REBUILD_READY || parent_pl_count <= 0) return;
        ...
        if (slot >= 0) {
            ...
            for (int word = 0; word < words; ++word) {
                ...
                while (bits) {
                    ...
                    int child_level = 1;
                    int child_rsize = 2;
                    int child_drop = (root_idx == root_pivot ? 1 : 0);
                    if (child_rsize - child_drop > target) continue;

                    unsigned long long* child_cl = pivot_cl_buffer(child_level);
                    make_kcgpu_child_cl(
                        enc, m, root_cl, root_pl, root_idx, word, child_cl);
                    ...
                    int child_pl_count = make_kcgpu_pl(
                        enc, m, child_cl, child_pivot, pivot_pl_buffer(child_level));
                    if (child_pl_count == 0) continue;

                    KPivotPrefixState root_state;
                    save_pivot_prefix_state(
                        &root_state,
                        child_level,
                        child_pl_count,
                        ...
                        child_cl,
                        words);
                    for (int s = 0; s < child_pl_count; s += GTAP_K_PIVOT_PREFIX_CHUNK) {
                        int e = s + GTAP_K_PIVOT_PREFIX_CHUNK;
                        if (e > child_pl_count) e = child_pl_count;
                        spawned = true;
                        #pragma gtap task
                        edgek_pivot_shared_prefix_task(
                            slot, u, v, m, child_level, root_state, s, e);
                    }
#else
