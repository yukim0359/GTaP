...
# Fixed colors to align with other Fibonacci scaling plots
COL_QUEUE1 = "#1f77b4"  # analogous to WS / baseline
COL_QUEUE3 = "#ff7f0e"  # analogous to GQ / alternative
COL_SPEEDUP = "#2ca02c"

csv_path = Path(f"daq_performance_results_{BENCHMARK_NAME}.csv")
out_path_comparison = Path(f"img/daq_comparison_{BENCHMARK_NAME}.{OUTPUT_FORMAT}")
out_path_speedup = Path(f"img/daq_speedup_{BENCHMARK_NAME}.{OUTPUT_FORMAT}")
out_path_combined = Path(f"img/daq_combined_{BENCHMARK_NAME}.{OUTPUT_FORMAT}")
