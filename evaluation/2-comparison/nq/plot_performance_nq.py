#!/usr/bin/env python3
"""Plot N-Queens: absolute time and GTaP-normalized time (combined figure)."""
...
if not csv_path.exists():
    raise SystemExit(f"Missing CSV: {csv_path} (run ./compare_nq.sh first)")
...
def configure_x_axis(ax, sub: pd.DataFrame):
    n_min = int(sub["n"].min())
    n_max = int(sub["n"].max())
    xticks = np.arange(n_min, n_max + 1)
    ax.set_xticks(xticks)
    ax.set_xticklabels([str(n) if (n % 2 == 0 or n >= 17) else "" for n in xticks])
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Plot N-Queens performance (GTaP vs CPU baselines).")
    ap.add_argument(
        "--include-seq",
        action="store_true",
