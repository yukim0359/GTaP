...
DAQ raises warp execution efficiency for fib by roughly 2-5 lanes, depending
...
time, especially for these short fib kernels. Use the existing DAQ timing CSVs
...
NQ does not show a meaningful active-lane improvement from DAQ. This supports
the interpretation that NQ's weak DAQ effect is not caused by the dispatch
...
CilkSort shows a small active-lane improvement from DAQ, about +1.1 lanes in
...
Interpretation: CilkSort is an intermediate case. DAQ improves SIMT lane
...
can execute as a full warp through the dispatch path. DAQ's observed scope is
...
- fib: clear improvement from DAQ.
- CilkSort: small improvement from DAQ.
- N-Queens: almost no improvement from DAQ.

This supports the current interpretation that DAQ helps when queue separation
