...
// All threads in the block participate via threadIdx.x-strided loops.
...
__device__ void tree_work(int node, int height, int mem_ops, int compute_iters) {
    if (height == 0) {
        // leaf: all threads participate in compute; thread 0 stores result
...
int main(int argc, char** argv) {
    ...
    printf("Tree workload (1 block = 1 task): height=%d nodes=%d mem_ops=%d compute_iters=%d\n",
    ...
    st = gtap_launch(exec_kernel, height, mem_ops, compute_iters);
    if (st != cudaSuccess) {
