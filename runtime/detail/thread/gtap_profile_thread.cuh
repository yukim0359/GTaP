#pragma once

#include <stdio.h>
#include <cuda_runtime.h>
#include <climits>
#include <sys/stat.h>
#include <sys/types.h>
#include <errno.h>
#ifdef _WIN32
#include <direct.h>
#endif

#ifdef GTAP_PROFILE

static inline void ensure_directory_exists(const char* path) {
    struct stat st = {0};
    if (stat(path, &st) == -1) {
        if (errno == ENOENT) {
            // Directory doesn't exist, create it
            #ifdef _WIN32
                _mkdir(path);
            #else
                mkdir(path, 0755);
            #endif
        }
    }
}

static inline void save_warp_having_task_time_to_csv(long long* having_task_time_data, int* indices, long long min_time, const char* app_name) {
    ensure_directory_exists("./profile");
    char timeline_path[256];
    snprintf(timeline_path, sizeof(timeline_path), "./profile/%s_warp_timeline_having_task.csv", app_name);
    FILE* timeline_file = fopen(timeline_path, "w");
    if (!timeline_file) {
        printf("Error: Could not create %s_warp_timeline_having_task.csv file\n", app_name);
        return;
    }
    fprintf(timeline_file, "warp_id,time_index,timestamp_ns,relative_time_ms,state,state_description\n");
    int totalWarps = gtap_stored_launch_config().total_workers;
    for (int warp = 0; warp < totalWarps; warp++) {
        int samples = indices[warp];
        if (samples == 0) continue;
        struct TimeEntry { long long timestamp; int index; int state; };
        struct TimeEntry* entries = (struct TimeEntry*)malloc(sizeof(struct TimeEntry) * samples);
        int entry_count = 0;
        for (int i = 0; i < samples; i++) {
            long long time = having_task_time_data[warp * gtap_profile_capacity() + i];
            if (time > 0) {
                entries[entry_count].timestamp = time;
                entries[entry_count].index = i;
                entries[entry_count].state = (i % 2 == 0) ? 1 : 0; // even:start(having), odd:end
                entry_count++;
            }
        }
        for (int i = 0; i < entry_count - 1; i++) {
            for (int j = i + 1; j < entry_count; j++) {
                if (entries[i].timestamp > entries[j].timestamp) {
                    struct TimeEntry temp = entries[i];
                    entries[i] = entries[j];
                    entries[j] = temp;
                }
            }
        }
        for (int i = 0; i < entry_count; i++) {
            double relative_time_ms = (entries[i].timestamp - min_time) / 1000000.0;
            const char* state_desc = (entries[i].state == 1) ? "Having" : "NotHaving";
            fprintf(timeline_file, "%d,%d,%lld,%.6f,%d,%s\n",
                    warp, entries[i].index, entries[i].timestamp,
                    relative_time_ms, entries[i].state, state_desc);
        }
        free(entries);
    }
    fclose(timeline_file);
    printf("\nWarp timeline saved to: %s_warp_timeline_having_task.csv\n", app_name);

    char stats_path[256];
    snprintf(stats_path, sizeof(stats_path), "./profile/%s_warp_statistics_having_task.csv", app_name);
    FILE* stats_file = fopen(stats_path, "w");
    if (!stats_file) {
        printf("Error: Could not create %s_warp_statistics_having_task.csv file\n", app_name);
        return;
    }
    fprintf(stats_file, "warp_id,total_samples,first_activity_ms,last_activity_ms,duration_ms\n");
    for (int warp = 0; warp < totalWarps; warp++) {
        int samples = indices[warp];
        if (samples == 0) {
            fprintf(stats_file, "%d,0,0.0,0.0,0.0\n", warp);
            continue;
        }
        int having = 0, not_having = 0;
        long long first_time = LLONG_MAX, last_time = 0;
        for (int i = 0; i < samples; i++) {
            long long time = having_task_time_data[warp * gtap_profile_capacity() + i];
            if (time > 0) {
                if (time < first_time) first_time = time;
                if (time > last_time) last_time = time;
                if (i % 2 == 0) having++; else not_having++;
            }
        }
        double first_activity_ms = (first_time - min_time) / 1000000.0;
        double last_activity_ms = (last_time - min_time) / 1000000.0;
        double duration_ms = (last_time - first_time) / 1000000.0;
        fprintf(stats_file, "%d,%d,%.6f,%.6f,%.6f\n",
                warp, having, first_activity_ms, last_activity_ms, duration_ms);
    }
    fclose(stats_file);
    printf("Warp statistics saved to: %s_warp_statistics_having_task.csv\n", app_name);
}

static inline void visualize_having_task_time(const char* app_name) {
    printf("\n=== Warp Having-Task Time Visualization ===\n");
    int totalWarps = gtap_stored_launch_config().total_workers;
    int* d_indices;
    cudaMalloc(&d_indices, sizeof(int) * totalWarps);
    get_final_warp_having_task_time_indices<<<totalWarps, 1>>>(d_indices);
    cudaDeviceSynchronize();

    int* indices = (int*)malloc(sizeof(int) * totalWarps);
    cudaMemcpy(indices, d_indices, sizeof(int) * totalWarps, cudaMemcpyDeviceToHost);

    long long* data = (long long*)malloc(sizeof(long long) * totalWarps * gtap_profile_capacity());
    if (!data) {
        printf("Error: Memory allocation failed for having_task_time_data\n");
        free(indices);
        cudaFree(d_indices);
        return;
    }
    cudaError_t st = get_warp_having_task_time_data(data);
    if (st != cudaSuccess) {
        printf("Error getting having-task time data: %s\n", cudaGetErrorString(st));
        free(indices);
        free(data);
        cudaFree(d_indices);
        return;
    }

    long long min_time = LLONG_MAX, max_time = 0;
    int total_samples = 0;
    for (int warp = 0; warp < totalWarps; warp++) {
        for (int i = 0; i < indices[warp]; i++) {
            long long t = data[warp * gtap_profile_capacity() + i];
            if (t > 0) {
                if (t < min_time) min_time = t;
                if (t > max_time) max_time = t;
                total_samples++;
            }
        }
    }
    if (total_samples == 0) {
        printf("No having-task time data recorded.\n");
        free(indices);
        free(data);
        cudaFree(d_indices);
        return;
    }

    printf("Total samples: %d\n", total_samples);
    printf("Time range: %.3f ms to %.3f ms\n", min_time / 1000000.0, max_time / 1000000.0);
    printf("Duration: %.3f ms\n", (max_time - min_time) / 1000000.0);

    int max_warps_to_show = (totalWarps < 10) ? totalWarps : 10;
    int timeline_width = 80;
    printf("\nTimeline visualization (first %d warps):\n", max_warps_to_show);
    printf("Warp  |");
    for (int i = 0; i < timeline_width; i++) printf((i % 10 == 0) ? "|" : "-");
    printf("\n");
    for (int warp = 0; warp < max_warps_to_show; warp++) {
        printf("%5d |", warp);
        char timeline[81];
        for (int i = 0; i < timeline_width; i++) timeline[i] = ' ';
        timeline[timeline_width] = '\0';
        for (int i = 0; i < indices[warp]; i++) {
            long long t = data[warp * gtap_profile_capacity() + i];
            if (t > 0) {
                int pos = (int)((t - min_time) * timeline_width / (max_time - min_time));
                if (pos >= 0 && pos < timeline_width) timeline[pos] = (i % 2 == 0) ? 'H' : 'N';
            }
        }
        printf("%s\n", timeline);
    }

    save_warp_having_task_time_to_csv(data, indices, min_time, app_name);
    free(indices);
    free(data);
    cudaFree(d_indices);
}

static inline void save_warp_working_time_to_csv(long long* working_time_data, int* indices, int* counts_data, long long min_time, const char* app_name) {
    ensure_directory_exists("./profile");
    char timeline_path[256];
    snprintf(timeline_path, sizeof(timeline_path), "./profile/%s_warp_timeline_working.csv", app_name);
    FILE* timeline_file = fopen(timeline_path, "w");
    if (!timeline_file) {
        printf("Error: Could not create %s_warp_timeline_working.csv file\n", app_name);
        return;
    }
    fprintf(timeline_file, "warp_id,time_index,timestamp_ns,relative_time_ms,state,state_description,tasks_in_batch\n");
    int totalWarps = gtap_stored_launch_config().total_workers;
    for (int warp = 0; warp < totalWarps; warp++) {
        int samples = indices[warp];
        if (samples == 0) continue;
        struct TimeEntry { long long timestamp; int index; int state; int count; };
        struct TimeEntry* entries = (struct TimeEntry*)malloc(sizeof(struct TimeEntry) * samples);
        int entry_count = 0;
        for (int i = 0; i < samples; i++) {
            long long time = working_time_data[warp * gtap_profile_capacity() + i];
            if (time > 0) {
                entries[entry_count].timestamp = time;
                entries[entry_count].index = i;
                entries[entry_count].state = (i % 2 == 0) ? 1 : 0; // even:pre, odd:post
                entries[entry_count].count = counts_data[warp * gtap_profile_capacity() + i];
                entry_count++;
            }
        }
        for (int i = 0; i < entry_count - 1; i++) {
            for (int j = i + 1; j < entry_count; j++) {
                if (entries[i].timestamp > entries[j].timestamp) {
                    struct TimeEntry temp = entries[i];
                    entries[i] = entries[j];
                    entries[j] = temp;
                }
            }
        }
        for (int i = 0; i < entry_count; i++) {
            double relative_time_ms = (entries[i].timestamp - min_time) / 1000000.0;
            const char* state_desc = (entries[i].state == 1) ? "Working" : "NotWorking";
            fprintf(timeline_file, "%d,%d,%lld,%.6f,%d,%s,%d\n",
                    warp, entries[i].index, entries[i].timestamp,
                    relative_time_ms, entries[i].state, state_desc, entries[i].count);
        }
        free(entries);
    }
    fclose(timeline_file);
    printf("\nWarp timeline saved to: %s_warp_timeline_working.csv\n", app_name);

    char stats_path2[256];
    snprintf(stats_path2, sizeof(stats_path2), "./profile/%s_warp_statistics_working.csv", app_name);
    FILE* stats_file = fopen(stats_path2, "w");
    if (!stats_file) {
        printf("Error: Could not create %s_warp_statistics_working.csv file\n", app_name);
        return;
    }
    fprintf(stats_file, "warp_id,total_samples,first_activity_ms,last_activity_ms,duration_ms,total_tasks\n");
    for (int warp = 0; warp < totalWarps; warp++) {
        int samples = indices[warp];
        if (samples == 0) {
            fprintf(stats_file, "%d,0,0.0,0.0,0.0,0\n", warp);
            continue;
        }
        int pre = 0, post = 0, total_tasks = 0;
        long long first_time = LLONG_MAX, last_time = 0;
        for (int i = 0; i < samples; i++) {
            long long time = working_time_data[warp * gtap_profile_capacity() + i];
            if (time > 0) {
                if (time < first_time) first_time = time;
                if (time > last_time) last_time = time;
                if (i % 2 == 0) pre++; else { post++; total_tasks += counts_data[warp * gtap_profile_capacity() + i]; }
            }
        }
        double first_activity_ms = (first_time - min_time) / 1000000.0;
        double last_activity_ms = (last_time - min_time) / 1000000.0;
        double duration_ms = (last_time - first_time) / 1000000.0;
        fprintf(stats_file, "%d,%d,%.6f,%.6f,%.6f,%d\n",
                warp, pre, first_activity_ms, last_activity_ms, duration_ms, total_tasks);
    }
    fclose(stats_file);
    printf("Warp statistics saved to: %s_warp_statistics_working.csv\n", app_name);
}

static inline void visualize_working_time(const char* app_name) {
    printf("\n=== Warp Working Time Visualization ===\n");
    int totalWarps = gtap_stored_launch_config().total_workers;
    int* d_indices;
    cudaMalloc(&d_indices, sizeof(int) * totalWarps);
    get_final_warp_working_time_indices<<<totalWarps, 1>>>(d_indices);
    cudaDeviceSynchronize();

    int* indices = (int*)malloc(sizeof(int) * totalWarps);
    cudaMemcpy(indices, d_indices, sizeof(int) * totalWarps, cudaMemcpyDeviceToHost);

    long long* working_time_data = (long long*)malloc(sizeof(long long) * totalWarps * gtap_profile_capacity());
    int* counts_data = (int*)malloc(sizeof(int) * totalWarps * gtap_profile_capacity());
    if (!working_time_data || !counts_data) {
        printf("Error: Memory allocation failed for working data\n");
        free(indices);
        free(working_time_data);
        free(counts_data);
        cudaFree(d_indices);
        return;
    }
    cudaError_t st1 = get_warp_working_time_data(working_time_data);
    cudaError_t st2 = get_warp_tasks_processed_count_data(counts_data);
    if (st1 != cudaSuccess || st2 != cudaSuccess) {
        printf("Error getting working data: %s, counts: %s\n", cudaGetErrorString(st1), cudaGetErrorString(st2));
        free(indices);
        free(working_time_data);
        free(counts_data);
        cudaFree(d_indices);
        return;
    }

    long long min_time = LLONG_MAX, max_time = 0;
    int total_samples = 0;
    for (int warp = 0; warp < totalWarps; warp++) {
        for (int i = 0; i < indices[warp]; i++) {
            long long t = working_time_data[warp * gtap_profile_capacity() + i];
            if (t > 0) {
                if (t < min_time) min_time = t;
                if (t > max_time) max_time = t;
                total_samples++;
            }
        }
    }
    if (total_samples == 0) {
        printf("No working time data recorded.\n");
        free(indices);
        free(working_time_data);
        free(counts_data);
        cudaFree(d_indices);
        return;
    }

    printf("Total samples: %d\n", total_samples);
    printf("Time range: %.3f ms to %.3f ms\n", min_time / 1000000.0, max_time / 1000000.0);
    printf("Duration: %.3f ms\n", (max_time - min_time) / 1000000.0);

    int max_warps_to_show = (totalWarps < 10) ? totalWarps : 10;
    int timeline_width = 80;
    printf("\nTimeline visualization (first %d warps):\n", max_warps_to_show);
    printf("Warp  |");
    for (int i = 0; i < timeline_width; i++) printf((i % 10 == 0) ? "|" : "-");
    printf("\n");
    for (int warp = 0; warp < max_warps_to_show; warp++) {
        printf("%5d |", warp);
        char timeline[81];
        for (int i = 0; i < timeline_width; i++) timeline[i] = ' ';
        timeline[timeline_width] = '\0';
        for (int i = 0; i < indices[warp]; i++) {
            long long t = working_time_data[warp * gtap_profile_capacity() + i];
            if (t > 0) {
                int pos = (int)((t - min_time) * timeline_width / (max_time - min_time));
                if (pos >= 0 && pos < timeline_width) timeline[pos] = (i % 2 == 0) ? 'W' : 'N'; // Working / NotWorking
            }
        }
        printf("%s\n", timeline);
    }

    save_warp_working_time_to_csv(working_time_data, indices, counts_data, min_time, app_name);
    free(indices);
    free(working_time_data);
    free(counts_data);
    cudaFree(d_indices);
}

void gtap_export_profile(const char* app_name) {
    visualize_having_task_time(app_name);
    visualize_working_time(app_name);
}

#endif
