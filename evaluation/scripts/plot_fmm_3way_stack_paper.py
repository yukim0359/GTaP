...
at ACM single-column width (3.33 in) with 8pt typography. Phases are collapsed
to seven paper segments (Tree/setup, DTT init, DTT, DTT H2D, M2L, P2P/L2P,
Other).
"""
...
def _default_output(n: int, theta: float) -> Path:
    theta_lbl = format_theta_label(theta)
    stem = f"fmm_3way_stack_N{n}_theta{theta_lbl}"
    return IMG_DIR / f"{stem}.{OUTPUT_FORMAT}"


def _figure_height(n_runs: int) -> float:
    plot_h = FIG_WIDTH_IN * (
...
def _plot_fmm_3way_stack_horizontal(
    ...
    *,
    ...
    fig_height_in = _figure_height(n_runs)
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help=f"Output path (default: img/fmm_3way_stack_N<N>_theta<THETA>.{OUTPUT_FORMAT}).",
    )
    parser.add_argument(
        "--no-init",
        action="store_true",
    ...
    output_path = (
        args.output or _default_output(args.n, args.theta)
    ).resolve()
