#!/usr/bin/env python3
"""
Plot worker scaling analysis for Fibonacci benchmark.
...
def add_ideal_scaling(ax, threads, times, label="Ideal Scaling", x_values=None):
    ...
    Uses a distinct linestyle ('-.') to avoid clashing with method lines.
    """
    threads = np.asarray(threads)
    times = np.asarray(times, dtype=float)

    # Find first valid point (smallest threads with positive time)
    valid = times > 0
    if not np.any(valid):
        return
...
def set_grouped_legend_2col(ax, first_prefix, second_prefix, ideal_label="Ideal Scaling", ncol=2):
    ...
    Force legend layout (2 columns) to be:
      Row 1: first_prefix (small block) | first_prefix (large block)
      Row 2: second_prefix (small block) | second_prefix (large block)
    ...
    # Expect two block sizes. If not, fall back to a simple ordering.
    if len(first) >= 2 and len(second) >= 2:
        ordered = []
        # column-major trick for ncol=2:
        # rows are (0,3), (1,4), (2,none) when N=5
    ...
    # Fallback: just sort by (group, block)
    def group_rank(lbl: str) -> int:
        if lbl.startswith(first_prefix): return 0
        if lbl.startswith(second_prefix): return 1
        if lbl == ideal_label: return 2
        return 3
...
# Ideal scaling: anchor at the smallest worker count of WS with block_size=32
...
# Ideal scaling: anchor at the smallest worker count of Ours with block_size=32 (within df_cl)
