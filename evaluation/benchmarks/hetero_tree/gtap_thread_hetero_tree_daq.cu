...
// Hetero tree (thread, w/ DAQ): leaf-only compute.
//   - expand (spawn) on queue 0 when child is internal
//   - leaf tasks on queue(1+kind)
//   - taskwait resumes on queue 0 (no internal compute)
...
__device__ void tree_work(int node, int height, int compute_iters) {
    ...
    #pragma gtap task queue((height == 1) ? kind_queue_idx(l) : 0)
    tree_work(l, height - 1, compute_iters);
    #pragma gtap task queue((height == 1) ? kind_queue_idx(r) : 0)
...
int main(int argc, char** argv) {
    ...
    printf("Hetero tree (thread, w/ DAQ K=%d queues=%d): height=%d nodes=%lld compute_iters=%d (out=%.2f GiB)\n",
           HETERO_TREE_K, HETERO_TREE_K + 1, height, (long long)total_nodes, compute_iters,
    ...
    gtap_thread_config config{
        ...
        .num_queues = HETERO_DAQ_NUM_QUEUES,
    };
    ...
    hetero_print_mix_stats("thread-DAQ");

#ifdef GTAP_ENABLE_PROFILING
    gtap_export_profile({
        .output_directory = "./profile/hetero_tree_thread_daq",
        .overwrite = true,
    });
#endif
