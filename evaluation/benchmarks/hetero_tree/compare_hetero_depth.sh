...
# Depth sweep for hetero_tree (4 impls): thread wo/DAQ, block, block-cutoff.
# Usage: qsub compare_hetero_depth.sh
#        or: bash compare_hetero_depth.sh
...
echo "Building hetero_tree binaries ..."
make -C "$COMPARE_DIR" all

# D sweep: compute_iters fixed (match binary_tree)
...
echo "n,GTAP_thread_med,GTAP_thread_err_low,GTAP_thread_err_high,GTAP_thread_daq_med,GTAP_thread_daq_err_low,GTAP_thread_daq_err_high,GTAP_block_med,GTAP_block_err_low,GTAP_block_err_high,GTAP_block_cutoff_med,GTAP_block_cutoff_err_low,GTAP_block_cutoff_err_high" > "$RESULTS_FILE"

printf "%6s | %25s | %25s | %25s | %25s\n" \
    "n" "thread wo (ms)" "thread DAQ (ms)" "block (ms)" "block cutoff (ms)"
printf "%6s-+-%25s-+-%25s-+-%25s-+-%25s\n" \
    "------" "-------------------------" "-------------------------" \
    "-------------------------" "-------------------------"
...
for depth in "${DEPTH_VALUES[@]}"; do
    ...
    echo "$depth,$TH_MED,$TH_ELO,$TH_EHI,$DAQ_MED,$DAQ_ELO,$DAQ_EHI,$BLK_MED,$BLK_ELO,$BLK_EHI,$CUT_MED,$CUT_ELO,$CUT_EHI" >> "$RESULTS_FILE"
done
