...
BENCHMARK_NAME = "tree"
TITLE_BENCHMARK_NAME = "Binary Tree (mem_ops=512, compute_iters=512)"
OUTPUT_FORMAT = "pdf"  # "png" or "pdf"

# Fixed colors to align with other EPAQ plots
...
# Relative time data: merge both dataframes on height to calculate relative time
speedup_df = pd.DataFrame()
if not queue1_df.empty and not queue2_df.empty:
    merged = pd.merge(queue1_df[["height", "1queue_med"]],
                     queue2_df[["height", "2queue_med"]],
                     on="height", how="inner")
    if not merged.empty:
