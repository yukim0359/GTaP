...
def _compute_utilization_from_timeline(timeline_df, strong_state, id_col):
    ...
    Args:
        ...
        id_col: Column name identifying workers (e.g., "warp_id", "block_id").
    
    Returns:
        DataFrame with columns [id_col, "utilization_percent"].
    """
    if (
        id_col not in timeline_df.columns
        or "relative_time_ms" not in timeline_df.columns
    ...
    for ident, grp in timeline_df.groupby(id_col):
        ...
        util = (
            max(0.0, min(100.0, (working_time / program_total_time) * 100.0))
            if program_total_time > 0.0
            else 0.0
        )
...
def _create_timeline_plot(
    ...
    if custom_title:
        ...
        ax.set_title("Worker Timeline Visualization: Pruned B-ary Tree")
...
def visualize_thread(custom_title=None):
    ...
    default_title = "Pruned B-ary Tree (Thread-level workers)"
...
def visualize_block(custom_title=None):
    ...
    default_title = "Pruned B-ary Tree (Block-level workers)"
    timeline_title = custom_title if custom_title else default_title
