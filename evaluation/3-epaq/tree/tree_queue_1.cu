...
__device__ double do_memory_or_compute_divergent(int node, int mem_ops, int compute_iters) {
    // const unsigned int warp_mask = __activemask(); // active lanes in this warp
    // const int lane_id = get_lane_id();

    const int node_type = hash32(node) % 3; // 0, 1, 2
    ...
    // // Error if the warp has mixed types among active lanes
    // if (lane_id == 0) {
    //     const int present = (type0_mask != 0) + (type1_mask != 0) + (type2_mask != 0);
    //     if (present > 1) {
    //         printf(
