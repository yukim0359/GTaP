...
# When submitted from gtap/evaluation/scheduler-comparison/$BENCHMARK_NAME
SCALING_DIR=$(pwd)
...
if [ ! -d "$RUNTIME_DIR" ]; then
    echo "Error: Please submit this script from gtap/evaluation/scheduler-comparison/$BENCHMARK_NAME"
    exit 1
fi
...
# Number of runs (matching compare_cilksort.sh)
NUM_RUNS=20

### cilksort-specific settings ###
CILKSORT_SIZE=20000000

# Prepare input data once
DATA_FILE="$TMP_DATA_DIR/cilksort_data_${CILKSORT_SIZE}.bin"
echo "Generating input data of size $CILKSORT_SIZE..."
cd "$UTIL_DIR"
make > /dev/null 2>&1
./gen_vector "$CILKSORT_SIZE" "$DATA_FILE" > /dev/null 2>&1
cd "$SCALING_DIR"

if [ ! -f "$DATA_FILE" ]; then
    echo "Error: Failed to generate data file at $DATA_FILE"
    exit 1
fi
...
# Function to compute median and IQR (matching compare_cilksort.sh)
...
for block_size in "${BLOCK_SIZES[@]}"; do
    ...
    for exp in "${TOTAL_THREADS_EXPONENTS[@]}"; do
        ...
        for i in $(seq 1 $NUM_RUNS); do
            output=$("$BINARY_WS_PATH" "$DATA_FILE" 2>&1 || true)
        ...
        TIMES_GQ=()
        for i in $(seq 1 $NUM_RUNS); do
            output=$("$BINARY_GQ_PATH" "$DATA_FILE" 2>&1 || true)
            time=$(echo "$output" | grep "Execution time" | sed 's/.*: \([0-9.]*\) ms.*/\1/')
            if [ -n "$time" ]; then
                TIMES_GQ+=("$time")
            fi
        ...
        TIMES_CL=()
        for i in $(seq 1 $NUM_RUNS); do
            output=$("$BINARY_CL_PATH" "$DATA_FILE" 2>&1 || true)
            time=$(echo "$output" | grep "Execution time" | sed 's/.*: \([0-9.]*\) ms.*/\1/')
            if [ -n "$time" ]; then
                TIMES_CL+=("$time")
            fi
