#!/usr/bin/env python3
"""Plot Fibonacci: absolute time and GTaP-normalized time (combined figure)."""
...
if not csv_path.exists():
    raise SystemExit(f"Missing CSV: {csv_path} (run ./compare_fib.sh first)")
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Plot Fibonacci performance (GTaP vs CPU baselines).")
    ap.add_argument(
        "--include-seq",
        action="store_true",
