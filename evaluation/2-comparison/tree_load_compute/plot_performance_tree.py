...
def main():
    ...
    if df_depth is not None:
        ...
        make_combined_plot(
            ...
            title="Varying height (mem_ops=256, compute_iters=256 fixed)",
        )
        ...
        if "depth" in df_mem_all.columns:
            for d in _depth_values(df_mem_all):
                ...
                make_combined_plot(
                    ...
                    title=f"Varying mem_ops (height={d}, compute_iters=256 fixed)",
                )
        else:
            make_combined_plot(
                df_mem_all,
                ...
                title="Varying mem_ops (height=20, compute_iters=256 fixed)",
            )
        ...
        if "depth" in df_comp_all.columns:
            for d in _depth_values(df_comp_all):
                ...
                make_combined_plot(
                    ...
                    title=f"Varying compute_iters (height={d}, mem_ops=256 fixed)",
                )
        else:
            make_combined_plot(
                df_comp_all,
                ...
                title="Varying compute_iters (height=20, mem_ops=256 fixed)",
            )
